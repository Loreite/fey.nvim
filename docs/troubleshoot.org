#+OPTIONS: H:9 ^:nil
* Troubleshooting
💡 TIP: Run ~:checkhealth fey~ to check if Fey is correctly installed and configured.

** Indentation is not working

Make sure you are not overriding indentexpr in Fey buffers with [[https://github.com/nvim-treesitter/nvim-treesitter#indentation][nvim-treesitter indentation]]

** I get ~treesitter/query.lua~ errors when opening agenda/capture prompt or fey files

Tree-sitter parser might not be installed.
Try running ~:Fey install_treesitter_grammar~ to reinstall it.

** I get =.../fey/parser/fey.so is not a valid Win32 application= on Windows
...
#+begin_src lua
require('fey.utils.treesitter.install').compilers = { 'clang' }
require('fey').setup()
#+end_src
After that, restart Neovim and run =:Fey install_treesitter_grammar= to reinstall the parser.
...
See related issue: [[https://github.com/loreite/fey.nvim/issues/879]]
...
#+END_SRC
More info on issue [[https://github.com/loreite/fey.nvim/issues/281#issuecomment-1120200775][#281]]
