...
:END:
Thanks for wanting to help out with fey.nvim, we appreciate the effort!
...
📣 Please always make a quick search in our [[https://github.com/Loreite/fey.nvim/issues][issue-tracker]] before
reporting anything. If the bug/feature has already been reported,
continue the conversation on the existing issue.

We distinguish between =core= (part of [[https://fey.fey/][fey]]) and
...
documented, have a look at these [[https://fey.fey/wfey/#resources][resources]]. The more info you provide the better!

** Documentation
:PROPERTIES:
:CUSTOM_ID: documentation
:END:
If you spot something missing in our [[file:./index.fey][docs]], don't hesitate making a PR.
The [[https://github.com/Loreite/fey.nvim/wiki][wiki]] can be edited freely.
...
:END:
We try to mimic the behavior of Emacs Fey as closely as possible.
Variable names follow the same naming convention as in Emacs Fey,
except that they are in =snake_case= instead of =kebab-case=.

Example: =fey-agenda-files= in Emacs is =fey_agenda_files= in fey.nvim.
...
- [[https://github.com/JohnnyMfeyanz/StyLua][StyLua]] - For formatting
...
:CUSTOM_ID: tests
:END:
To run tests run =make test= in the fey.nvim directory:
...
:CUSTOM_ID: parser
:END:
Parsing is done via builtin treesitter parser and the [[https://github.com/Loreite/tree-sitter-fey][tree-sitter-fey]] grammar.

**** Commits
:PROPERTIES:
:CUSTOM_ID: commits
:END:
Ensure that you follow the [[https://www.conventionalcommits.fey/][Conventional Commits]] specification (e.g., =feat: add new feature=, =fix: correct bug=, =docs: update documentation=).
