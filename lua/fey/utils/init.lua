...
---@return FeyPromise<integer> bytes
...
function utils.open_window(name, height, split_mode, border)
  ...
  if cmd_by_split_mode[split_mode] then
    ...
    vim.w.fey_window_split_mode = split_mode
    return
  end
  ...
  if split_mode == 'auto' then
    ...
    if (winwidth / 2) >= 80 then
      ...
      vim.w.fey_window_split_mode = 'vertical'
    else
      vim.cmd(cmd_by_split_mode.horizontal)
      vim.w.fey_window_split_mode = 'horizontal'
    end
    return
  end
...
function utils.open_tmp_fey_window(height, split_mode, border, on_close)
  local prev_winnr = vim.api.nvim_get_current_win()
  utils.open_window(vim.fn.tempname() .. '.fey', height or 16, split_mode, border)
  vim.cmd([[setlocal filetype=fey bufhidden=wipe nobuflisted nolist noswapfile nofoldenable]])
  local bufnr = vim.api.nvim_get_current_buf()
  local augroup = vim.api.nvim_create_augroup('FeyTmpWindow_' .. bufnr, { clear = true })
...
function utils.edit_file(filename)
  ...
  return {
    open = function()
      ...
      vim.api.nvim_buf_set_var(bufnr, 'fey_tmp_edit_window', true)
      vim.api.nvim_open_win(bufnr, true, {
        relative = 'editor',
        width = 1,
...
---@param filename string
---@return boolean
function utils.is_fey_file(filename)
  local ext = vim.fn.fnamemodify(filename, ':e')
  return ext == 'fey' or ext == 'fey_archive'
end
