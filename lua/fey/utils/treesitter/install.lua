...
function M.get_version_info()
  ...
  -- if not result.installed_in_fey_dir then
  --   return result
  -- end
  --
...
-- function M.get_parser_locations()
--   local runtime_files = vi/m.api.nvim_get_runtime_file('parser/fey.so', true)
...
function M.get_lock_file()
  return vim.fs.joinpath(M.get_package_path(), '.fey-ts-lock.json')
end
...
function M.get_path(url, type)
  ...
  return M.exe('git', {
    ...
    if code ~= 0 then
      error('[fey] Failed to clone tree-sitter-fey: ' .. (tostring(err) or 'Unknown error'), 0)
    end
    return path
  end)
end
...
function M.run(type)
  ...
  if not compiler then
    error('[fey] No C compiler found for installing tree-sitter grammar', 0)
  end
  ...
  return M.get_path(url, type)
    ...
    :next(function(code, err)
      if code ~= 0 then
        error('[fey] Failed to compile parser: ' .. (tostring(err) or 'Unknown error'), 0)
      end
