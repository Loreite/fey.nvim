...
function M.parse_current_file()
  return vim.treesitter.get_parser(0, 'fey', {}):parse()
end
...
function M.find_heading(node)
  ...
  if node:parent() then
    return M.find_heading(node:parent())
  end
...
function M.find_item_or_heading(node)
  ...
  if node_type == 'heading' or node_type == 'listitem' then
    return node
  end
  ...
  if node_type == 'list' then
    ...
    vim.cmd([[norm l]])
    return M.find_item_or_heading(M.get_node_at_cursor())
  end
...
function M.closest_item_or_heading_node(cursor)
  return M.find_item_or_heading(M.get_node_at_cursor(cursor))
end
