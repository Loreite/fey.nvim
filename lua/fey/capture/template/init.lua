...
---@field heading? string|fun(FeyFile?):string
---@field regexp? string
...
function Template:_validate_regexp()
  if self.heading and self.regexp then
    local desc = self.description ~= '' and self.description or self.template
    utils.echo_error(('Cannot use both heading and regexp options in the same capture template "%s"'):format(desc))
  end
end
