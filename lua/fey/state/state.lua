...
---@class FeyState
local FeyState = { data = {}, _ctx = { loaded = false, saved = false, curr_loader = nil, savers = 0, dirty = false } }

local cache_path = vim.fs.normalize(vim.fn.stdpath('cache') .. '/fey-cache.json', { expand_env = false })

---Returns the current FeyState singleton
---@return FeyState
function FeyState.new()
  -- This is done so we can later iterate the 'data'
  -- subtable cleanly and shove it into a cache
  setmetatable(FeyState, {
...
---@return FeyPromise
function FeyState:save()
  if not FeyState._ctx.dirty then
    return Promise.resolve(self)
  end
  FeyState._ctx.saved = false
  --- We want to ensure the state was loaded before saving.
  self:load()
  self._ctx.savers = self._ctx.savers + 1
  return utils
    .writefile(cache_path, vim.json.encode(FeyState.data))
    :next(function()
      self._ctx.savers = self._ctx.savers - 1
      if self._ctx.savers == 0 then
        FeyState._ctx.saved = true
        FeyState._ctx.dirty = false
      end
    end)
...
function FeyState:save_sync(timeout)
  FeyState._ctx.saved = false
  self:save()
  vim.wait(timeout or 500, function()
    return FeyState._ctx.saved
  end, 20)
end

---Load the state cache into the current state
---@return FeyPromise
function FeyState:load()
  ...
  --- potential to have two FeyState:load operations occuring and whichever
    ...
    :next(function(data)
      ...
      if not success then
        ...
        vim.schedule(function()
          utils.echo_warning('FeyState cache load failure, error: ' .. vim.inspect(err_msg))
...
---@return FeyState
function FeyState:load_sync(timeout)
  local state
  local err
  self
    :load()
  ...
  if err == nil and state == nil then
    error('Did not load FeyState in time')
  end
...
function FeyState:wipe(overwrite)
