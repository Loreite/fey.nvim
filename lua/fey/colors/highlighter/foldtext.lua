...
---@alias FeyFoldtextLineValue false | { col: number, hl_group: string }

---@class FeyFoldtextHighlighter
---@field highlighter FeyHighlighter
---@field namespace number
---@field cache table<number, table<number, FeyFoldtextLineValue>>
---@field cache_tick table<number, number>
local FeyFoldtext = {}

---@param opts { highlighter: FeyHighlighter }
function FeyFoldtext:new(opts)
  local data = {
...
---@param bufnr number
function FeyFoldtext:check_cache(bufnr)
...
---@param bufnr number
---@param line number
---@param value FeyFoldtextLineValue
function FeyFoldtext:_highlight(bufnr, line, value)
  if not value then
    return
  end
  ...
  vim.api.nvim_buf_set_extmark(bufnr, self.highlighter.namespace, line, value.col, {
    hl_mode = 'combine',
    virt_text = { { config.fey_ellipsis, value.hl_group } },
    virt_text_pos = 'overlay',
    ephemeral = self.highlighter._ephemeral,
  })
end

function FeyFoldtext:on_line(bufnr, line, winid)
...
function FeyFoldtext:on_detach(bufnr)
  self.cache[bufnr] = nil
