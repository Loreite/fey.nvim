local colors = require('fey.colors.colors')
local namespace = vim.api.nvim_create_namespace('fey_agenda')
local M = {}
...
---@param highlights FeyHighlightEntry[]
---@param clear? boolean
---@return string
...
---@param virt_texts{ range: FeyRange, content: string, virt_text_pos: string, hl_groups: string[] }[]
---@param bufnr number
...
M.clear_extmarks = function(bufnr, start_line, end_line)
  local extmarks = vim.api.nvim_buf_get_extmarks(bufnr, namespace, { start_line, 0 }, { end_line, 99999 }, { details = true })
...
M.add_hr = function(bufnr, line, separator)
  ...
  vim.api.nvim_buf_set_extmark(bufnr, namespace, line, 0, {
    virt_text = { { string.rep(separator, width), '@fey.agenda.separator' } },
