...
---@return boolean
function link_utils.goto_oneof_headings(headings, file_path, error_message)
  if #headings == 0 then
  ...
  if #headings == 1 then
    utils.goto_heading(headings[1])
    return true
  end

  local longest_heading = utils.reduce(headings, function(acc, h)
    return math.max(acc, h:get_heading_line_content():len())
  end, 0)
  local options = {}
  for i, h in ipairs(headings) do
    table.insert(
      options,
      string.format('%d) %-' .. longest_heading .. 's (%s)', i, h:get_heading_line_content(), h.file.filename)
    )
  end
  vim.cmd([[echo "Multiple targets found. Select target:"]])
  local choice = vim.fn.inputlist(options)
  if choice < 1 or choice > #headings then
    return true
  end

  utils.goto_heading(headings[choice])
  return true
end
