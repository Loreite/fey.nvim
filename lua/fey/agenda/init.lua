...
function Agenda:render()
  ...
  for i, view in ipairs(self.views) do
    ...
    if #self.views > 1 and i < #self.views and #view:get_lines() > 0 then
      colors.add_hr(bufnr, vim.api.nvim_buf_line_count(bufnr), config.fey_agenda_block_separator)
    end
  end
  vim.bo[bufnr].modifiable = false

  if vim.w.fey_window_split_mode == 'horizontal' then
    local win_height = math.max(math.min(34, vim.api.nvim_buf_line_count(bufnr)), config.fey_agenda_min_height)
    if vim.w.fey_window_pos and vim.deep_equal(vim.fn.win_screenpos(0), vim.w.fey_window_pos) then
      vim.cmd(string.format('resize %d', win_height))
      vim.w.fey_window_pos = vim.fn.win_screenpos(0)
    else
      vim.w.fey_window_pos = nil
    end
  end
end
...
function Agenda:_build_custom_commands()
  if not config.fey_agenda_custom_commands then
    return {}
  end
  local custom_commands = {}
  ---@param opts FeyAgendaCustomCommandType
  local get_type_opts = function(opts, id)
    local opts_by_type = {
      agenda = {
        span = opts.fey_agenda_span,
        start_day = opts.fey_agenda_start_day,
        start_on_weekday = opts.fey_agenda_start_on_weekday,
      },
      tags = {
        match_query = opts.match,
        todo_ignore_scheduled = opts.fey_agenda_todo_ignore_scheduled,
        todo_ignore_deadlines = opts.fey_agenda_todo_ignore_deadlines,
      },
      tags_todo = {
        match_query = opts.match,
        todo_ignore_scheduled = opts.fey_agenda_todo_ignore_scheduled,
  ...
  for shortcut, command in utils.sorted_pairs(config.fey_agenda_custom_commands) do
...
function Agenda:_open_window()
  ...
  utils.open_window('feyagenda', math.max(34, config.fey_agenda_min_height), config.win_split_mode, config.win_border)

  vim.cmd([[setf feyagenda]])
...
function Agenda:switch_to_item()
  local item = self:_get_heading()
  if not item then
    return
  end
  utils.goto_heading(item)
end

function Agenda:change_todo_state()
  return self:_remote_edit({
    action = 'fey_mappings.todo_next_state',
...
function Agenda:preview_item()
  local heading = self:get_heading_at_cursor()
  if not heading then
    return
  end

  local lines = heading:get_lines()
  local offset = 4
...
function Agenda:priority_up()
  return self:_remote_edit({
    action = 'fey_mappings.priority_up',
    update_in_place = true,
  })
end
...
function Agenda:priority_down()
  return self:_remote_edit({
    action = 'fey_mappings.priority_down',
    update_in_place = true,
  })
end
...
function Agenda:toggle_archive_tag()
  return self:_remote_edit({
    action = 'fey_mappings.toggle_archive_tag',
    update_in_place = true,
  })
end
...
function Agenda:set_deadline()
  return self:_remote_edit({
    action = 'fey_mappings.fey_deadline',
    redo = true,
  })
end
...
function Agenda:set_schedule()
  return self:_remote_edit({
    action = 'fey_mappings.fey_schedule',
    redo = true,
  })
end
...
---@private
---@return FeyHeading | nil, FeyAgendaLine | nil, FeyAgendaViewType | nil
function Agenda:_get_heading()
  local line = vim.fn.line('.')
  for _, view in ipairs(self.views) do
    local agenda_line = view:get_line(line)
    if agenda_line and agenda_line.heading then
      return agenda_line.heading, agenda_line, view
    end
  end
end
...
function Agenda:_remote_edit(opts)
  ...
  local getter = opts.getter
    or function()
      local heading, agenda_line, view = self:_get_heading()
      if not heading then
        return
      end
      return heading, agenda_line, view
    end
  local heading, agenda_line, view = getter()
  if not heading then
    return
  end
...
function Agenda:get_heading_at_cursor()
  local line_nr = vim.fn.line('.')

  for _, view in ipairs(self.views) do
    local agenda_line = view:get_line(line_nr)
    if agenda_line and agenda_line.heading then
      return agenda_line.heading
    end
  end
end
