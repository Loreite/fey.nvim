...
---@alias FeyAgendaSortingStrategy
---| 'time-up'
---| 'time-down'
---| 'priority-down'
---| 'priority-up'
...
---@class SortableEntry
---@field date FeyDate Available only in agenda view
...
function SortingStrategy.priority_down(a, b)
  ...
  if a.heading:get_priority_sort_value() ~= b.heading:get_priority_sort_value() then
    return a.heading:get_priority_sort_value() > b.heading:get_priority_sort_value()
  end
...
function SortingStrategy.todo_state_up(a, b)
  ...
  local _, _, _, a_index = a.heading:get_todo()
  local _, _, _, b_index = b.heading:get_todo()
...
function SortingStrategy.category_up(a, b)
  ...
  if a.heading.file:get_category() ~= b.heading.file:get_category() then
    return a.heading.file:get_category() < b.heading.file:get_category()
  end
end
...
function SortingStrategy.category_keep(a, b)
  ...
  if a.heading.file.index ~= b.heading.file.index then
    return a.heading.file.index < b.heading.file.index
  end
end
...
function SortingStrategy.clocked_up(a, b)
  if a.heading:is_clocked_in() and not b.heading:is_clocked_in() then
    return true
  end
  if not a.heading:is_clocked_in() and b.heading:is_clocked_in() then
    return false
  end
end
...
---@generic T
---@param items T[]
---@param strategies FeyAgendaSortingStrategy[]
