...
---@field private _bufnrs table<integer, FeyVirtualIndent> Buffers with VirtualIndent attached
local VirtualIndent = {
  _ns_id = vim.api.nvim_create_namespace('fey.ui.indent'),
  _bufnrs = {},
}
...
function VirtualIndent.toggle_buffer_indent_mode(bufnr)
  ...
  local message = ''
  if vim.b[bufnr].fey_indent_mode then
    message = 'disabled'
    instance:detach()
  else
    message = 'enabled'
  ...
  require('fey.utils').echo_info('Fey-Indent mode ' .. message .. ' in current buffer')
end
...
function VirtualIndent:_get_indent_size(line, tree_has_errors)
  ...
  -- Fallback to searching closest heading by checking each previous line
  if tree_has_errors then
    ...
    while linenr > 0 do
      ...
      if level then
        -- If the current line is a heading we should return no virtual indentation, otherwise
  ...
  local heading = tree_utils.closest_heading_node({ line + 1, 1 })

  if heading then
    local heading_line = heading:start()

    if heading_line ~= line then
      local _, level = heading:field('signature')[1]:end_()
      return level + 1
    end
  end
...
function VirtualIndent:set_indent(start_line, end_line, ignore_ts)
  ...
  local heading = tree_utils.closest_heading_node({ start_line + 1, 1 })
  if heading and not ignore_ts then
    local parent = heading:parent()
    if parent then
  ...
  for line = start_line, end_line do
    ...
    if indent > 0 then
      ...
      pcall(vim.api.nvim_buf_set_extmark, self._bufnr, self._ns_id, line, 0, {
        virt_text = { { string.rep(' ', indent), 'FeyIndent' } },
...
function VirtualIndent:attach()
  ...
  self._attached = true
  vim.b[self._bufnr].fey_indent_mode = true
end
...
function VirtualIndent:detach()
  ...
  vim.b[self._bufnr].fey_indent_mode = false
end
