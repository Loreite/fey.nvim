...
local es_utils = require('fey.objects.edit_special.utils')
local utils = require('fey.utils')

local EditSpecialSrc = {
  extmark_ns = vim.api.nvim_create_namespace('fey_edit_special_extmark'),
  hl_ns = vim.api.nvim_create_namespace('fey_edit_special_hl'),
}
...
function EditSpecialSrc:_update_content(action, block_start_line, content)
  local block_indent = string.rep(vim.opt.expandtab:get() and ' ' or '\t', config.fey_edit_src_content_indentation)
  ...
  local block_start_line_text =
    vim.api.nvim_buf_get_lines(self.fey_bufnr, block_start_line, block_start_line + 1, false)[1]
...
function EditSpecialSrc:clear_highlights()
  vim.api.nvim_buf_clear_namespace(self.fey_bufnr, self.hl_ns, 0, -1)
end

function EditSpecialSrc:_highlight_contents(range)
  self:clear_highlights()
  local start_row, start_col, end_row, end_col = unpack(range)
  vim.highlight.range(self.fey_bufnr, self.hl_ns, 'FeyEditSrcHighlight', { start_row, start_col }, { end_row, end_col })
end
...
function EditSpecialSrc:write(ctx)
  ...
  vim.api.nvim_buf_set_lines(ctx.fey_bufnr, content_start, content_end, false, new_content)

  self.file:reload()

  -- If this is after the special buffer has been closed, our extmarks will have already
  -- been removed
  local new_start_extmark = vim.api.nvim_buf_get_extmark_by_id(ctx.fey_bufnr, ctx.extmark_ns, ctx.start_extmark, {})
  local new_end_extmark = vim.api.nvim_buf_get_extmark_by_id(ctx.fey_bufnr, ctx.extmark_ns, ctx.end_extmark, {})
