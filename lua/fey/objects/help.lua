...
function Help.show(type)
  ...
  vim.api.nvim_buf_set_name(Help.buf, 'feyhelp')
  vim.api.nvim_set_option_value('filetype', 'feyhelp', { buf = Help.buf })
  ...
  local fey_help_augroup = vim.api.nvim_create_augroup('fey_help', { clear = true })
  vim.api.nvim_create_autocmd('BufWipeout', {
    buffer = Help.buf,
    group = fey_help_augroup,
    callback = function()
      require('fey.objects.help').dispose()
    end,
  })
end
...
function Help.prepare_content(buffer_type)
  local opts_by_type = {
    fey = { title = 'Fey', include_fey_mappings = false },
    capture = { title = 'Capture', include_fey_mappings = true },
    note = { title = 'Note', include_fey_mappings = true },
    agenda = { title = 'Agenda', include_fey_mappings = false },
    edit_src = { title = 'Edit src', include_fey_mappings = false },
  }
  ...
  if opts_by_type[buffer_type].include_fey_mappings then
    vim.list_extend(content, Help._generate_mappings('fey', opts_by_type.fey.title))
    vim.list_extend(content, Help._generate_mappings('text_objects', 'Text Objects'))
    title = title .. ' + Fey'
  elseif buffer_type == 'fey' then
    vim.list_extend(content, Help._generate_mappings('text_objects', 'Text Objects'))
  end
  ...
  return vim.list_extend({
    string.format(' **Fey mappings - %s:%s**', title, scroll_more_text),
    '',
  }, content)
end
