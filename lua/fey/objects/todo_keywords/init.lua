...
local TodoKeyword = require('fey.objects.todo_keywords.todo_keyword')

---@class FeyTodoKeywords
---@field fey_todo_keywords string[][]|string[]
---@field fey_todo_keyword_faces table<string, string>
---@field todo_keywords FeyTodoKeyword[]
---@field sequences FeyTodoKeyword[][] Array of todo keyword sequences
local TodoKeywords = {}
TodoKeywords.__index = TodoKeywords

---@param opts { fey_todo_keywords: string[][]|string[], fey_todo_keyword_faces: table<string, string> }
---@return FeyTodoKeywords
function TodoKeywords:new(opts)
  -- Support both single sequence (string[]) and multiple sequences (string[][])
  local normalized_keywords = opts.fey_todo_keywords
  if type(normalized_keywords[1]) ~= 'table' then
    normalized_keywords = { normalized_keywords }
  end

  local this = setmetatable({
    fey_todo_keywords = normalized_keywords,
    fey_todo_keyword_faces = opts.fey_todo_keyword_faces,
    sequences = {},
  }, self)
  this:_parse()
  return this
...
---@return table<string, FeyTodoKeyword>
...
---@param type FeyTodoKeywordType
---@return FeyTodoKeyword
...
function TodoKeywords:_parse()
  ...
  for seq_idx, sequence in ipairs(self.fey_todo_keywords) do
...
function TodoKeywords:_create_keyword(keyword, status_type, index, seq_idx, used_shortcuts)
  ...
  elseif not used_shortcuts[todo_keyword.shortcut] and #self.fey_todo_keywords > 1 then
...
---@return FeyTodoKeyword[] keywords for the sequence
---@return FeyTodoKeyword[] seq_keywords keywords in this sequence
...
function TodoKeywords:_get_hl(keyword, type)
  if not self.fey_todo_keyword_faces[keyword] then
    return type == 'TODO' and '@fey.keyword.todo' or '@fey.keyword.done'
  end
  return ('@fey.keyword.face.%s'):format(keyword:gsub('%-', ''))
end
