...
local namespace = vim.api.nvim_create_namespace('fey_calendar')
local colors = require('fey.colors')
local Range = require('fey.files.elements.range')
local Input = require('fey.ui.input')

---@alias FeyCalendarOnRenderDayOpts { line: number, from: number, to: number, buf: number, namespace: number }
---@alias FeyCalendarOnRenderDay fun(day: FeyDate, opts: FeyCalendarOnRenderDayOpts)
...
---@field win number?
---@field buf number?
---@field callback fun(date: FeyDate | nil, cleared?: boolean)
---@field date FeyDate?
---@field title? string
---@field on_day? FeyCalendarOnRenderDay
---@field selected FeyDate?
...
vim.cmd([[hi default FeyCalendarToday gui=reverse cterm=reverse]])
vim.cmd([[hi default FeyCalendarSelected gui=underline cterm=underline]])

---@param data { date?: FeyDate, clearable?: boolean, title?: string, on_day?: FeyCalendarOnRenderDay }
...
function Calendar:open()
  ...
  vim.api.nvim_buf_set_name(self.buf, 'feycalendar')
  self.win = vim.api.nvim_open_win(self.buf, true, get_window_opts())

  local calendar_augroup = vim.api.nvim_create_augroup('fey_calendar', { clear = true })
...
function Calendar:on_render_day(day, opts)
  if day:is_today() then
    self:_apply_hl('FeyCalendarToday', opts.line - 1, opts.from - 1, opts.to)
  end
  if day:is_same_day(self.date) then
    self:_apply_hl('FeyCalendarSelected', opts.line - 1, opts.from - 1, opts.to)
  end
  if self.on_day then
    self.on_day(
      day,
