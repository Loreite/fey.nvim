...
local Heading = require('fey.files.heading')
...
---@field metadata FeyFileMetadata
...
function FeyFile.load(filename)
  local bufnr = Buffers.get_buffer_by_filename(filename)

  if bufnr > -1 and vim.api.nvim_buf_is_loaded(bufnr) and vim.bo[bufnr].filetype == 'fey' then
    return Promise.resolve(FeyFile:new({
      filename = filename,
      buf = bufnr,
    }))
  end
...
---@param term string
---@return FeyHeading[]
function FeyFile:find_headings_with_property_matching(property_name, term)
  return vim.tbl_filter(function(item)
...
---Find headings where property value is matching the term exactly
---@param property_name string
---@param term string
---@return FeyHeading[]
function FeyFile:find_headings_with_property(property_name, term)
  return vim.tbl_filter(function(item)
    local property = item:get_property(property_name)
...
function FeyFile:set_node_text(node, text, front_trim)
  ...
  -- for example a single line heading on line 10 gives
...
function FeyFile._load_buffer(filename)
...
function FeyFile:_get_parser()
  local bufnr = self:bufnr()

  if bufnr > -1 then
    -- Always get the fresh parser for the buffer
    return ts.get_parser(bufnr, 'fey', {})
  end
  ...
  if not self.parser or self:is_modified() or type(self.parser:source()) == 'number' then
    return ts.get_string_parser(self.content, 'fey', {})
  end
